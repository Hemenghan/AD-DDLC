...
path=".checkpoint/cifar10/wrn34_10_rn_18/RSLAD/ori/"
...
for epoch in range(1,epochs+1):
    for step,(train_batch_data,train_batch_labels) in enumerate(trainloader):
        ...
        if step%100 == 0:
            print(str(epoch)+'loss',loss.item())
    ...
    new_row = pd.DataFrame({'epoch': [epoch], 'adv_accuracy': [test_acc], 'timestamp': [current_time]})
    ...
    if epoch in [115,160,185]:
        for param_group in optimizer.param_groups:
            param_group['lr'] *= 0.1

print("best robust acc",best_test_acc)
print("best robust epoch",best_epoch)