...
import numpy as np
from L_sm_align import *
def get_classifier_weight(model):
    """
    支持 DataParallel 和常见命名（fc, classifier, linear）
    返回 Tensor of shape [num_classes, channels]
    """
    m = model.module if isinstance(model, torch.nn.DataParallel) else model
    if hasattr(m, 'fc'):
        return m.fc.weight
    if hasattr(m, 'classifier'):
        return m.classifier.weight
    if hasattr(m, 'linear'):
        return m.linear.weight
    raise AttributeError("Cannot find classifier weight (fc/classifier/linear) in model")
